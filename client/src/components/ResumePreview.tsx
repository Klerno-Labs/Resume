...
function parseResumeText(text: string): ParsedResume {
  ...
  if (nonEmptyLines[1] && !nonEmptyLines[1].includes('@') && !nonEmptyLines[1].match(/^\d/) && nonEmptyLines[1].length < 60) {
  ...
  const sectionKeywords = [
    ...
    'summary', 'objective', 'achievements', 'awards', 'professional',
    'work', 'technical', 'core competencies', 'qualifications'
  ];
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const lineLower = trimmedLine.toLowerCase().replace(/[:\-–—_]/g, ' ').trim();
    ...
    }) && trimmedLine.length < 40 && !trimmedLine.startsWith('-') && !trimmedLine.startsWith('•');
      ...
      <div className={`bg-white text-slate-800 font-sans ${className}`}>
        <div className="space-y-1">
          {parsed.rawLines.map((line, idx) => {
            ...
            if (isName) {
              return (
                <h1 key={idx} className="text-xl font-bold text-slate-900 text-center pb-2 border-b border-slate-200 mb-3">
                  {line}
                </h1>
              );
            }
            ...
            if (isHeader) {
              return (
                <h2 key={idx} className="text-xs font-bold uppercase tracking-widest text-blue-600 border-b border-blue-200 pb-1 mt-4 mb-2">
                  {line}
                </h2>
              );
            }
            ...
            if (isBullet) {
              return (
                <div key={idx} className="flex gap-2 text-[11px] text-slate-700 leading-relaxed pl-4">
          ...
          <div key={idx}>
            ...
            <div className="space-y-1">
              {section.content.map((line, lineIdx) => {
                ...
                if (isBullet) {
                  return (
                    <div key={lineIdx} className="flex gap-2 text-[11px] text-slate-700 leading-relaxed pl-3">
                ...
                if (hasDate && hasSeparator) {
                  ...
                  return (
                    <div key={lineIdx} className="flex justify-between items-baseline mt-2 first:mt-0 gap-2">
                  ...
                  if (colonIdx > 0) {
                    return (
                      <div key={lineIdx} className="text-[11px] leading-relaxed">
                        <span className="font-semibold text-slate-900">{line.substring(0, colonIdx)}:</span>
...
export function ResumePreviewStyled({ text, className = "" }: ResumePreviewProps) {
  ...
  return (
    <div className={`bg-white text-slate-800 p-10 min-h-full ${className}`} style={{ fontFamily: 'Georgia, "Times New Roman", serif' }}>
      {/* Elegant Header */}
      <div className="text-center pb-5 mb-6 border-b-2 border-slate-300">
        ...
        {parsed.title && (
          <p className="text-sm text-slate-600 tracking-widest mt-2">
            {parsed.title}
          </p>
        )}
        {parsed.contactLine && (
      ...
      <div className="space-y-5">
        {parsed.sections.length > 0 ? (
            ...
            <div key={idx}>
              ...
              <div className="space-y-1.5 pl-1">
                {section.content.map((line, lineIdx) => {
                  ...
                  if (isBullet) {
                    return (
                      <div key={lineIdx} className="flex gap-3 text-[11px] text-slate-700 leading-relaxed pl-2">
                  ...
                  if (hasDate) {
                    return (
                      <p key={lineIdx} className="text-[11px] text-slate-800 leading-relaxed font-semibold mt-2 first:mt-0">
                        {line}
                      </p>
                    );
                  }
          ...
          parsed.rawLines.slice(1).map((line, idx) => {
            ...
            if (isHeader) {
              return (
                <h2 key={idx} className="text-sm font-bold uppercase tracking-[0.2em] text-slate-700 border-b border-slate-400 pb-1 mt-4 mb-3">
                  {line}
                </h2>
              );
            }
            ...
            if (isBullet) {
              return (
                <div key={idx} className="flex gap-3 text-[11px] text-slate-700 leading-relaxed pl-2">
                  <span className="text-slate-400">◆</span>
                  <span>{line.replace(/^[-•*]\s*/, '')}</span>
                </div>
              );
