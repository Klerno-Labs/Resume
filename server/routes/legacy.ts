...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (allowedMimes.includes(file.mimetype)) {
      ...
      console.log(
        `[FileFilter] Accepting file with MIME type fallback: ${file.originalname} (MIME: ${file.mimetype})`
      );
      cb(null, true);
    } else {
      cb(
        new Error(
          `Invalid file type. Only PDF, DOCX, DOC, and TXT files are allowed. Received: ${file.mimetype}`
        )
      );
    }
  },
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        console.log(
          `[Credit] Deducted 1 credit from user ${userId}. Remaining: ${userAfterDeduction.creditsRemaining}`
        );
  ...
  app.get('/api/design/templates', requireAuth, async (req, res) => {
    try {
      ...
      const requestedFileKey =
        typeof req.query.fileKey === 'string' ? req.query.fileKey : undefined;
